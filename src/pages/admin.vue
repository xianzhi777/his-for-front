    ...
    <el-dialog
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="权限" :label-width="formLabelWidth" prop="status">
          <el-select v-model="ruleForm.status"   placeholder="请选择权限"  >
            <el-option  label="医生" :value="5"></el-option>
            <el-option  label="护士" :value="6"></el-option>
            <el-option  label="药房人员" :value="7"></el-option>
          </el-select>
        </el-form-item>
...
tableData.slice((currentPage-1)*pagesize,currentPage*pagesize)"
      >
        ...
        <el-table-column label="员工账户" prop="account"></el-table-column>
        <el-table-column label="员工姓名" prop="adminName"></el-table-column>
        <el-table-column label="员工地址" prop="address"></el-table-column>
        <el-table-column label="员工部门" prop="deptName"></el-table-column>
        <el-table-column label="员工职称" prop="levelName"></el-table-column>
        <el-table-column label="员工民族" prop="nationName"></el-table-column>
        <el-table-column label="员工手机" prop="phoneNum"></el-table-column>
        <el-table-column label="员工性别" prop="sex"></el-table-column>
        <el-table-column label="权限" prop="status" >
          <template scope="scope">
          <p v-if="scope.row.status=='5'">医生</p>
          <p v-if="scope.row.status=='6'">护士</p>
          <p v-if="scope.row.status=='7'">药房人员</p>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button @click="deleteuser(scope.row.adminId)" type="text" size="small">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      findAll () {
        axios.get('/back/admin/allStaff').then((res) => {
          this.tableData = res
        })
      },
      handleClose (done) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if(this.dialogType==1){
              axios.post('/back/admin/addStaff',this.ruleForm).then((res) => {
                if (res <= 0) {
            ...
            if(this.dialogType==0){
              axios.put('/back/admin/updStaff',this.ruleForm).then((res) => {
                if(res>0){
      ...
      deleteuser (id) {
        this.$confirm('确定删除？').then(_ => {
          axios.delete('back/admin/delStaff/' + id).then((res) => {
            if (res <= 0) {
